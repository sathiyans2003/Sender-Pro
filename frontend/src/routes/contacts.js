...
router.get('/groups', protect, async (req, res) => {
  const groups = await Contact.distinct('group', { userId: req.user.id });
  res.json(groups);
});
...
router.delete('/:id', protect, async (req, res) => {
  await Contact.findOneAndDelete({ id: req.params.id, userId: req.user.id });
...
router.delete('/', protect, async (req, res) => {
  const { ids } = req.body;
  await Contact.deleteMany({ id: { $in: ids }, userId: req.user.id });
