...
router.get('/', protect, async (req, res) => {
  const campaigns = await Campaign.find({ userId: req.user.id }).sort('-createdAt');
  res.json(campaigns);
});
...
router.post('/:id/start', protect, async (req, res) => {
  const campaign = await Campaign.findOne({ id: req.params.id, userId: req.user.id });
...
router.delete('/:id', protect, async (req, res) => {
  await Campaign.findOneAndDelete({ id: req.params.id, userId: req.user.id });
